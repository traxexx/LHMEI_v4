...
// constructor
ProperDeck::ProperDeck( const int win_len, const int short_clip, const int min_clip,
	const int min_read_len, const int min_qual, vector<RefSeq*> & Ref_Seq ):
	deck_start(0),
	WinLen(win_len),
	ShortClip(short_clip),
	MinClip(min_clip),
	MinReadLen(min_read_len),
	MinQuality(min_qual),
	rSeq(Ref_Seq)
{
	properDeck.resize(WinLen);
}
...
// locate first
	...
	if (dist >= WinLen * 2 - 1) { // start new deck
		deck_start = sam_rec.get1BasedPosition();
		properDeck.clear();
		properDeck.resize(WinLen);
		it_deck = properDeck.begin();
	}
	else if ( dist >= WinLen ) { // pop & push
		int pop_len = dist - WinLen + 1;
		deck_start += pop_len;
...
// some additional sanity check
	if (deck_dist < 0 || deck_dist >= WinLen) {
//		if (DEBUG_MODE)
